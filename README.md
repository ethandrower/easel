...
- **Sketch first** — press `S` (or **✎ sketch**) and click the canvas to drop a rough frame with no HTML at all: a title and a few lines of notes (`## region`, `- what lives there`, `? open question`) rendered as a low-fi wireframe in a real screen's footprint. Link sketches into flows, annotate them, rearrange — then **⇧ Promote to design** scaffolds the HTML and opens the Claude prompt seeded with the notes. Scope a whole module before a single screen is designed.
