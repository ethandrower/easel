...
- **Sketch first (Balsamiq-style wireframes)** — press `S` (or **✎ sketch**) and click the canvas to drop a frame with no HTML at all, then build it from a palette of primitives: box, heading, text, button, input, select, checkbox, table, image, tabs. Drag, resize, double-click to label; pin numbered **notes** to specific elements (drag a note's ◉, leader lines show what describes what) saying how each piece works. **⇧ Promote to design** scaffolds the HTML and hands Claude the full spec — element geometry *plus* the located annotations — and the wireframe stays for re-imagining later.
- **Faces per screen** — double-click any frame for full screen with a face bar: flip between the **✏ wireframe**, the built **design**, and each lettered **iteration** of the same screen.
- **Storyboard notes** — every design frame carries an annotation strip right under it: click, type the details and intent for that screen, and they're saved to the view and fed into that screen's Claude prompt. The storyboard *is* the spec.
- **Iterations per screen** — hit **iterate** on any view to stack lettered design directions (B, C, …) of the same screen, each linked back to its base and prompted to diverge deliberately. Keep the winner, delete the rest.
- **Style-library sync + lint** — point `canvas.config.json` at your app's real style sources (a Tailwind config and/or component CSS): `easel styles sync` pulls tokens + classes into the canvas (zero build) and tells every Claude prompt exactly which classes exist; `easel styles lint` flags inline styles, rules that duplicate a library class, invented library-look-alike classes, and custom styles repeated across screens.
