...
- [ ] **Sketch mode** — a rough-first layer for scoping a new module before any real design exists: drop low-fi frames (title + a few bullet regions, no HTML authoring) straight from the canvas, drag them into flows with documentation arrows, annotate with post-its, then "fill in" any frame with the Claude design prompt when it's ready. Needs: a `_sketch.html` low-fi template (dashed regions, question/phase chips) alongside `_template.html`; a quick-add that takes just a title + bullets; a "promote to design" action that swaps the sketch for the real prompt flow. Prototyped by hand on the citemed canvas (Position module, 2026-08-28).
