...
**Sketches / wireframes**: a view's `view.json` may carry `"sketch": { "elements": [...] }` — a wireframe. Each element is `{ id, type, x, y, w, h, label }` in a 1200×780 frame; types: box, heading, text, button, input, select, checkbox, table, image, tabs, and **note**. A `note` element is a numbered annotation: its `el` field pins it to another element's id (null = about the whole screen) and its `label` is the annotation text. When building or revising the screen's `index.html`, treat the wireframe as the layout spec and the pinned notes as behavior requirements — match the relative geometry (not pixel-exactly), satisfy every note, and say what you chose where a note asks a question. NEVER delete `sketch`: the wireframe and the design coexist as two faces of the same screen. (Older prose sketches use `"sketch": {"text": "..."}` with `## region` / `- item` / `? question` lines — same intent; move that text to `notes` when you build the screen.)
